...
LibavStreamer::LibavStreamer(const async_web_server_cpp::HttpRequest &request,
    ...
    : ImageTransportImageStreamer(request, connection, nh), output_format_(0),
      format_context_(0), codec_(0), codec_context_(0), video_stream_(0),
      frame_(0), sws_context_(0), first_image_timestamp_(0),
      format_name_(format_name), codec_name_(codec_name),
...
static int dispatch_output_packet(void *opaque, uint8_t *buffer,
                                  int buffer_size) {
  async_web_server_cpp::HttpConnectionPtr connection =
...
void LibavStreamer::initialize(const cv::Mat &img) {
  ...
  if (!format_context_) {
    async_web_server_cpp::HttpReply::stock_reply(
        ...
        request_, connection_, NULL, NULL);
  ...
  AVIOContext *io_ctx =
      avio_alloc_context(io_buffer_, io_buffer_size, AVIO_FLAG_WRITE,
...
void LibavStreamer::sendImage(const cv::Mat &img, const ros::Time &time) {
  ...
  avpicture_fill(raw_frame, img.data, input_coding_format, output_width_,
                 output_height_);
#else
  AVFrame *raw_frame = av_frame_alloc();
  av_image_fill_arrays(raw_frame->data, raw_frame->linesize, img.data,
                       input_coding_format, output_width_, output_height_, 1);
#endif
  ...
  int ret = sws_scale(sws_context_, (const uint8_t *const *)raw_frame->data,
                      raw_frame->linesize, 0, output_height_, frame_->data,
                      frame_->linesize);
...
boost::shared_ptr<ImageStreamer> LibavStreamerType::create_streamer(
  ...
  return boost::shared_ptr<ImageStreamer>(new LibavStreamer(
      request, connection, nh, format_name_, codec_name_, content_type_));
}
