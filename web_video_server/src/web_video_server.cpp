...
static bool
ros_connection_logger(async_web_server_cpp::HttpServerRequestHandler forward,
...
WebVideoServer::WebVideoServer(ros::NodeHandle &nh, ros::NodeHandle &private_nh)
    : nh_(nh), handler_group_(async_web_server_cpp::HttpReply::stock_reply(
                   async_web_server_cpp::HttpReply::not_found)) {
#if ROS_HAS_STEADYTIMER || defined USE_STEADY_TIMER
  cleanup_timer_ = nh.createSteadyTimer(
      ros::WallDuration(0.5),
      boost::bind(&WebVideoServer::cleanup_inactive_streams, this));
#else
  cleanup_timer_ = nh.createTimer(
      ros::Duration(0.5),
      boost::bind(&WebVideoServer::cleanup_inactive_streams, this));
#endif
  ...
  private_nh.param<std::string>("default_stream_type", __default_stream_type,
                                "mjpeg");

  stream_types_["mjpeg"] =
      boost::shared_ptr<ImageStreamerType>(new MjpegStreamerType());
  stream_types_["png"] =
      boost::shared_ptr<ImageStreamerType>(new PngStreamerType());
  stream_types_["ros_compressed"] =
      boost::shared_ptr<ImageStreamerType>(new RosCompressedStreamerType());
  stream_types_["vp8"] =
      boost::shared_ptr<ImageStreamerType>(new Vp8StreamerType());
  stream_types_["h264"] =
      boost::shared_ptr<ImageStreamerType>(new H264StreamerType());
  stream_types_["vp9"] =
      boost::shared_ptr<ImageStreamerType>(new Vp9StreamerType());
  ...
  try {
    ...
    ROS_ERROR("Exception when creating the web server! %s:%d", address_.c_str(),
              port_);
    throw;
  }
}
...
void WebVideoServer::restreamFrames(double max_age) {
  ...
  for (itr_type itr = image_subscribers_.begin();
       itr < image_subscribers_.end(); ++itr) {
...
void WebVideoServer::cleanup_inactive_streams() {
  ...
  if (lock) {
    typedef std::vector<boost::shared_ptr<ImageStreamer>>::iterator itr_type;
    itr_type new_end =
        std::partition(image_subscribers_.begin(), image_subscribers_.end(),
...
bool WebVideoServer::handle_stream(
    ...
    const char *end) {
  std::string type =
      request.get_query_param_value_or_default("type", __default_stream_type);
  if (stream_types_.find(type) != stream_types_.end()) {
    ...
    if (type == std::string("ros_compressed")) {
      ...
      if (!did_find_compressed_topic) {
        ROS_WARN_STREAM("Could not find compressed image topic for "
                        << topic << ", falling back to mjpeg");
        type = "mjpeg";
      }
    }
    boost::shared_ptr<ImageStreamer> streamer =
        stream_types_[type]->create_streamer(request, connection, nh_);
    streamer->start();
    boost::mutex::scoped_lock lock(subscriber_mutex_);
    image_subscribers_.push_back(streamer);
  } else {
    async_web_server_cpp::HttpReply::stock_reply(
        async_web_server_cpp::HttpReply::not_found)(request, connection, begin,
                                                    end);
  }
  return true;
}
...
bool WebVideoServer::handle_snapshot(
    ...
    const char *end) {
  boost::shared_ptr<ImageStreamer> streamer(
      new JpegSnapshotStreamer(request, connection, nh_));
  streamer->start();
...
bool WebVideoServer::handle_list_streams(
    ...
    async_web_server_cpp::HttpConnectionPtr connection, const char *begin,
    const char *end) {
  std::string image_message_type =
      ros::message_traits::datatype<sensor_msgs::Image>();
  std::string camera_info_message_type =
      ros::message_traits::datatype<sensor_msgs::CameraInfo>();

  std::string slot_prefix = "";
  char const *tmp = getenv("SLOT_PREFIX");

  if (tmp != NULL) {
    slot_prefix = "/" + std::string(tmp);
  }
    ...
    if (boost::algorithm::ends_with(camera_info_topic, "/camera_info")) {
      ...
      for (; image_topic_itr != image_topics.end();) {
        if (boost::starts_with(*image_topic_itr, base_topic)) {
          connection->write("<li><a href=\"" + slot_prefix +
                            "/stream_viewer?topic=");
          connection->write(*image_topic_itr);
          connection->write("\">");
          connection->write(image_topic_itr->substr(base_topic.size()));
          connection->write("</a> (");
          connection->write("<a href=\"" + slot_prefix + "/snapshot?topic=");
          connection->write(*image_topic_itr);
  ...
  std::vector<std::string>::iterator image_topic_itr = image_topics.begin();
  for (; image_topic_itr != image_topics.end();) {
    connection->write("<li><a href=\"" + slot_prefix + "/stream_viewer?topic=");
    connection->write(*image_topic_itr);
    connection->write("\">");
    connection->write(*image_topic_itr);
    connection->write("</a> (");
