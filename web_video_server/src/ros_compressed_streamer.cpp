...
void RosCompressedStreamer::start() {
  ...
  image_sub_ = nh_.subscribe(compressed_topic, 1,
                             &RosCompressedStreamer::imageCallback, this);
}
...
void RosCompressedStreamer::restreamFrame(double max_age) {
  ...
  if (last_frame + ros::Duration(max_age) < ros::Time::now()) {
    ...
    sendImage(last_msg, ros::Time::now()); // don't update last_frame, it may
                                           // remain an old value.
  }
}

void RosCompressedStreamer::sendImage(
    const sensor_msgs::CompressedImageConstPtr &msg, const ros::Time &time) {
  try {
...
void RosCompressedStreamer::imageCallback(
  ...
  boost::mutex::scoped_lock lock(
      send_mutex_); // protects last_msg and last_frame
  last_msg = msg;
...
boost::shared_ptr<ImageStreamer> RosCompressedStreamerType::create_streamer(
  ...
  return boost::shared_ptr<ImageStreamer>(
      new RosCompressedStreamer(request, connection, nh));
}
