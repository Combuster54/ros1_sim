...
ImageTransportImageStreamer::ImageTransportImageStreamer(
  ...
  default_transport_ =
      request.get_query_param_value_or_default("default_transport", "raw");
}
...
void ImageTransportImageStreamer::start() {
  ...
  for (size_t it = 0; it < available_topics.size(); it++) {
    ...
    if (available_topic_name == topic_ ||
        (available_topic_name.find("/") == 0 &&
         available_topic_name.substr(1) == topic_)) {
      inactive_ = false;
    }
  }
  image_sub_ = it_.subscribe(
      topic_, 1, &ImageTransportImageStreamer::imageCallback, this, hints);
}
...
void ImageTransportImageStreamer::restreamFrame(double max_age) {
  ...
  try {
    if (last_frame + ros::Duration(max_age) < ros::Time::now()) {
      ...
      sendImage(output_size_image,
                ros::Time::now()); // don't update last_frame, it may remain an
                                   // old value.
    }
