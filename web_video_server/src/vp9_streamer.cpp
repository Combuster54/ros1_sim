...
Vp9Streamer::Vp9Streamer(const async_web_server_cpp::HttpRequest &request,
                         ...
                         ros::NodeHandle &nh)
    : LibavStreamer(request, connection, nh, "webm", "libvpx-vp9",
                    "video/webm") {}
Vp9Streamer::~Vp9Streamer() {}

void Vp9Streamer::initializeEncoder() {

  // codec options set up to provide somehow reasonable performance in cost of
  // poor quality should be updated as soon as VP9 encoding matures
  ...
  av_opt_set_int(codec_context_->priv_data, "crf", 20,
                 0); // 0..63 (higher is lower quality)
}
...
boost::shared_ptr<ImageStreamer> Vp9StreamerType::create_streamer(
  ...
  return boost::shared_ptr<ImageStreamer>(
      new Vp9Streamer(request, connection, nh));
}
